...
class Constants(BaseConstants):
    ...
    firm_PF = c(np.random.choice([50, 0], 1, p=[0.1, 0.9]).item(0))  # paritally comply when fully inspected
    ...
    firm_NL = c(np.random.choice([60, 10], 1, p=[0.5, 0.5]).item(0))  # not comply when lightly inspected
    ...
    insp_PF = c(-30 if firm_PF==c(50) else 20)  # full inspection when paritally comply
    ...
    insp_NL = c(0 if firm_NL==c(60) else 50)  # light inspection when not comply
